...
const NginxLinkInstructions = () => {
  ...
  const cacheControlCode = sprintf(
    `map $sent_http_x_wp_cf_super_cache_active $wp_cf_super_cache_active {
              ...
              {isToggleOn('cf_cache_control_htaccess') && (
                <>
                  ...
                  <div className="space-y-4">
                    ...
                    <Notice
                      type="info"
                      title={__('Why $sent_http_ instead of $upstream_http_?', 'wp-cloudflare-page-cache')}
                      description={createInterpolateElement(
                        sprintf(
                          // translators: %s is a code tag.
                          __('The %s variable is used instead of %s because the latter is only populated when Nginx contacts the PHP/FastCGI backend. If your server also runs an Nginx-level page cache in front of PHP (FastCGI Cache, %s, and similar setups used by some managed hosts), cached hits are served directly by Nginx and never reach PHP, leaving that variable empty. %s reflects the header that is actually about to be sent to the visitor, whether it was just generated by PHP or replayed from the Nginx cache, so %s stays correct in both cases.', 'wp-cloudflare-page-cache'),
                          '<code>$sent_http_</code>',
                          '<code>$upstream_http_</code>',
                          '<code>proxy_cache</code>',
                          '<code>$sent_http_</code>',
                          '<code>Cache-Control</code>'
                        ),
                        {
                          code: <code>$sent_http_</code>
                        }
                      )}
                    />
                    <Notice
                      type="warning"
                      title={__('Multi-tier reverse proxy setups', 'wp-cloudflare-page-cache')}
                      description={createInterpolateElement(
                        sprintf(
                          // translators: %s is a code tag.
                          __('If your Nginx page cache runs on a separate node in front of this web server (for example a dedicated caching/reverse-proxy tier), cache hits there never reach this configuration. Apply the same map and %s rules on that caching node instead, since it is the one deciding what gets served for cache hits.', 'wp-cloudflare-page-cache'),

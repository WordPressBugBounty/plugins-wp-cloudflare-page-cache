...
define( 'SWCFPC_ADVANCED_CACHE_VERSION', 'SWCFPC_VERSION_PLACEHOLDER' );
...
function swcfpc_fallback_cache_end( $html ) {
	...
	if ( ! is_callable( [ $sw_cloudflare_pagecache, 'get_core_loader' ] ) ) {
		return $html;
	}

	$core_loader = $sw_cloudflare_pagecache->get_core_loader();

	if ( ! is_callable( [ $core_loader, 'fallback_cache' ] ) ) {
		return $html;
	}

	$fallback_cache = $core_loader->fallback_cache();
...
/**
 * Get the manifest of known subsite paths for the current host, as generated by
 * the plugin's multisite path discovery process.
 *
 * @return array<string, array<int, string>>
 */
function swcfpc_get_multisite_paths_manifest() {
	...
	$file = WP_CONTENT_DIR . '/wp-cloudflare-super-page-cache/multisite-paths.json';
...
/**
 * Resolve the subdirectory path for a multisite subsite based on the
 * current request URI and the known paths manifest.
 *
 * @param string $host Bare host (no port), as returned by swcfpc_get_host_without_port().
 *
 * @return string
 */
function swcfpc_resolve_multisite_subdirectory_path( $host ) {
	$manifest = swcfpc_get_multisite_paths_manifest();
	...
	foreach ( $manifest[ $host ] as $known_path ) {
		if ( ! is_string( $known_path ) || '' === $known_path ) {
			continue;
		}

		$is_match = $request_path === $known_path || 0 === strpos( $request_path, $known_path . '/' );

		if ( $is_match && strlen( $known_path ) > strlen( $best_match ) ) {
	...
	return preg_replace( '/[^A-Za-z0-9_-]+/', '-', $best_match );
}
...
function swcfpc_get_cache_base_path() {
	...
	$subsite_suffix = swcfpc_resolve_multisite_subdirectory_path( $bare_host );

	if ( '' !== $subsite_suffix ) {
		$subsite_path = WP_CONTENT_DIR . "/wp-cloudflare-super-page-cache/{$bare_host}_{$subsite_suffix}";

		if ( file_exists( "{$subsite_path}/main_config.php" ) ) {
			return $subsite_path;
		}
	}
...
function swcfpc_fallback_cache_is_cookie_to_exclude() {
	...
	if (
		...
		! isset( $swcfpc_config['cf_fallback_cache_excluded_cookies'] ) ||
		! is_array( $swcfpc_config['cf_fallback_cache_excluded_cookies'] )
	) {
		return false;
	}

...
- **Introduced Cache Tags (PRO)** with `Cache-Tag` header generation, tag indexing, automatic tag-based purging, async processing, REST and WP-CLI tools, and admin bar inspection support.
...
- **Added encrypted settings storage**, WP-CLI management tools, and `wp-config.php` overrides for plugin settings.
...
- **Updated Apache and Nginx cache instructions** to better support gzip/Brotli and improve `Cache-Control` handling for XML/XSL assets.
